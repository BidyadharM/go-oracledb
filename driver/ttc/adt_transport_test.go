...
func TestRefCursorOutBindUsesCursorOAC(t *testing.T) {
	...
	normalized := normalizeBindValue(sql.Out{Dest: &refcursor.RefCursor{}})
...
func TestGetObjectTypeBindsUnusedMetadataAsRefCursors(t *testing.T) {
	...
	_, err := adt.GetObjectType(context.Background(), adtMetadataExecer(func(_ context.Context, _ string, args ...any) (sql.Result, error) {
		if len(args) != 10 {
		...
		for _, index := range []int{8, 9} {
			...
			if _, ok := out.Dest.(*refcursor.RefCursor); !ok {
				t.Fatalf("bind %d destination = %T, want *refcursor.RefCursor", index+1, out.Dest)
			}
		}
		return nil, sentinel
