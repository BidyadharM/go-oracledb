...
func (cap *Capability) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	...
	for i := range cap.compileTimeCapabilities {
		...
		if err != nil {
			...
			return common.NewOracleError(common.FailUnmarshal, err, "Capability")
		}
...
func (cap *Capability) MarshalTo(ctx context.Context, mar common.Marshaller) error {
	...
	if err := mar.MarshalB1Array(ctx, (common.B1Array)(cap.compileTimeCapabilities)); err != nil {
		common.Odl.Warn("Failed to marshal compile time capabilities", "error", err)
		return common.NewOracleError(common.FailMarshal, err, "Capability")
	}

	if err := mar.MarshalUB1(ctx, common.UB1(len(cap.runTimeCapabilities))); err != nil {
		common.Odl.Warn("Failed to marshal runtime capabilities", "error", err)
		return common.NewOracleError(common.FailMarshal, err, "Capability")
	}

	if err := mar.MarshalB1Array(ctx, (common.B1Array)(cap.runTimeCapabilities)); err != nil {
		common.Odl.Warn("failed to marshal runtime capabilities", "error", err)
		return common.NewOracleError(common.FailMarshal, err, "Capability")
	}
	return nil
...
func capabilitiesToMap(capabilities []byte, metadata map[string]capabilityMetadata, capabilityMap map[string]common.Capability) {
