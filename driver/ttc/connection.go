...
func (c *Connection) cancelCurrentExecution(ctx context.Context) error {
	if err := c.ns.CancelOperation(ctx); err != nil {
		...
		c._isValid = false
		e := common.NewOracleError(common.CancelOperationError, err, nil)
...
func checkNamedValue(nv *driver.NamedValue) error {
	...
	if out, ok := nv.Value.(sql.Out); ok {
		...
		if destInfo.Kind() != reflect.Ptr {
			return common.NewOracleError(common.InvalidSqlOutParameter, errors.New("non pointer"))
		}

		// Destination pointer must not be nil.
		if destInfo.IsNil() {
			return common.NewOracleError(common.InvalidSqlOutParameter, errors.New("nil pointer"))
		}
		...
		if pointedValue.Kind() == reflect.Ptr {
			return common.NewOracleError(common.InvalidSqlOutParameter, errors.New("double pointer"))
		}
...
func (c *Connection) _registerServerTimezoneOffset(ctx context.Context) error {
	...
	if err != nil {
		return c.shelf.LocalizeError(common.NewOracleError(common.ServerTimeZoneError, err, "query"))
	}
	defer rows.Close()
	...
	if err := rows.Next(values); err != nil {
		return c.shelf.LocalizeError(common.NewOracleError(common.ServerTimeZoneError, err, "retrieve"))
	}
...
func parseTimeZone(timezone string) (int, int, error) {
	...
	if len(items) != 2 {
		return 0, 0, common.NewOracleError(common.ServerTimeZoneError, nil, "parse")
	}

	TZH, err := strconv.Atoi(items[0])
	if err != nil {
		return 0, 0, common.NewOracleError(common.ServerTimeZoneError, err, "parse")
	}
