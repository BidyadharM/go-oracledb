...
func TestAuthencationFactoryBasic(t *testing.T) {
	t.Parallel()
	c := common.NewOracleDriverConfig()
...
func TestGetConnection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ciConnectionProps := &common.OracleDriverProperties{}
...
func TestGetConnectionMissingLocalizationService(t *testing.T) {
	...
	if sqle.ErrorCode() != string(common.MissingLocalizationService) {
		t.Fatalf("expected error code %s, got %s", common.MissingLocalizationService, sqle.ErrorCode())
	}
