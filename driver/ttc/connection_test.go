...
func init() {
	message.SetString(language.French, string(common.InternalError), "erreur interne factice.")
	message.SetString(language.French, string(common.StatementExecutionFailed), "echec factice de preparation de %s: %s.")
}
...
func TestConnection_ParseTimeZoneRejectsMalformedValues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if sqlErr, ok := err.(common.SQLError); !ok || sqlErr.ErrorCode() != string(common.ServerTimeZoneError) {
				t.Fatalf("parseTimeZone(%q) should return %s, got %T: %v", tt.input, common.ServerTimeZoneError, err, err)
			}
...
func TestNewConnectionReturnsServerTimezoneError(t *testing.T) {
	...
	if sqlErr.ErrorCode() != string(common.ServerTimeZoneError) {
		t.Fatalf("expected error code %s, got %s", common.ServerTimeZoneError, sqlErr.ErrorCode())
	}
	if conn != nil {
...
func TestConnection_ExecContext_LocalizesError(t *testing.T) {
	...
	if sqlErr.ErrorCode() != string(common.StatementExecutionFailed) {
		t.Fatalf("expected error code %s, got %s", common.StatementExecutionFailed, sqlErr.ErrorCode())
	}
...
func TestConnection_FaultyOnDrain(t *testing.T) {
	...
	connection := newTestConnection(shelf, common.NewSessionContext(), mockNs)
...
func (m *connInvalidationMsg) GetMsgCode() common.MessageType { return m.msgType }
func (m *connInvalidationMsg) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
	return nil
}
