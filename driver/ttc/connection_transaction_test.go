	...
	for _, testItem := range tests {
		...
		if sqlErr, ok := err.(common.SQLError); ok {
			if sqlErr.ErrorCode() != string(common.NotInTransaction) {
				t.Errorf("Wrong error, expected %s but was %s", common.NotInTransaction, sqlErr.ErrorCode())
			}
		}
...
func TestCallBeginTxTwice(t *testing.T) {
	...
	oracleError, ok := err.(common.SQLError)
	if !ok {
		t.Fatalf("Expected error to be OracleError")
	}
	if oracleError.ErrorCode() != string(common.AlreadyInTransaction) {
		t.Fatalf("Wrong error expected %s, but was %s", common.AlreadyInTransaction, oracleError.ErrorCode())
	}
