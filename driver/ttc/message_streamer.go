...
type StreamerPreUnmarshallCallback func(*messageHeader) (common.Message[common.MessageType], error)
...
type MessageStreamer struct {
	...
	postUCallbacks map[common.MessageType]StreamerPostUnmarshallCallback
	preUCallbacks  map[common.MessageType]StreamerPreUnmarshallCallback
}
...
func NewMessageStreamer(shelf *ttiShelf[common.MessageType]) *MessageStreamer {
	return &MessageStreamer{
		incomingMessages: list.New(),
		outgoingMessages: list.New(),
		preUCallbacks:    make(map[common.MessageType]StreamerPreUnmarshallCallback),
		postUCallbacks:   make(map[common.MessageType]StreamerPostUnmarshallCallback),
		shelf:            shelf,
	}
}

// Push implementation. See Streamer interface
func (ms *MessageStreamer) Push(ctx context.Context, msg common.Message[common.MessageType]) error {
	...
	if ms.outgoingMessages.Len() > outgoingMessagesListMaxLength {
		...
		if err != nil {
			...
			ms.Drain(common.BackgroundContext, common.OUT)
		}
	}
...
func _isExpectedType(ts []common.MessageType, t common.MessageType) bool {
	for _, v := range ts {
		if v == t {
			return true
		}
...
func (ms *MessageStreamer) Pull(ctx context.Context, expectedMessageTypes ...common.MessageType) (common.Message[common.MessageType], error) {
	...
	for {
		...
		if errHeader != nil {
			...
			return nil, common.NewOracleError(common.StreamerReadError, errHeader, nil)
		}
		...
		if msg == nil {
			...
			if ferr != nil {
				...
				return nil, common.NewOracleError(common.StreamerReadError, ferr, nil)
			}
		}
		marshallable, isUnMarshallable := msg.(common.UnMarshallable)
		if !isUnMarshallable {
			...
			return nil, common.NewOracleError(common.StreamerReadError, nil, nil)
		}
		...
		if ms.incomingMessages.Len() >= incomingMessagesListMaxLength {
			...
			return nil, common.NewOracleError(common.InternalError, nil, nil)
		}
...
func (ms *MessageStreamer) Flush(ctx context.Context) error {
	...
	for outgoing := ms.outgoingMessages.Front(); outgoing != nil; outgoing = next {
		next = outgoing.Next()
		outgoingMsg := outgoing.Value.(common.Message[common.MessageType])
		marshalable, _ := outgoingMsg.(common.Marshallable)
		...
		if err != nil {
			...
			return common.NewOracleError(common.StreamerWriteError, err, nil)
		}
...
func (ms *MessageStreamer) Drain(ctx context.Context, direction common.StreamDirection) (int, int) {
	if common.Odl.Enabled(common.BackgroundContext, slog.LevelDebug) {
	...
	var iRes = -1
	var oRes = -1
	if direction == common.IN || direction == common.INOUT {
		iRes = ms.incomingMessages.Len()
		ms.incomingMessages.Init()
	}
	if direction == common.OUT || direction == common.INOUT {
...
func (ms *MessageStreamer) getMessageForHeader(header *messageHeader) (common.Message[common.MessageType], error) {
