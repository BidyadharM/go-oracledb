...
const (
	nclobReadOffset = common.UB8(6)
	nclobTestPhrase = "こんにちは – नमस्ते – Привет – مرحبا – 你好 – 😀 "
)

var (
	nclobExpectedString = strings.Repeat(nclobTestPhrase, 10)
	nclobReadNumChars   = common.UB8(32523) //common.UB8(len([]rune(nclobExpectedString)))
)
...
func TestClobExecutor_WriteNCLOB(t *testing.T) {
	...
	locator := make(common.B1Array, len(nclobWriteLocator))
	...
	if written != common.UB8(expectedCodeUnits) {
...
var nclobReadLocator = common.B1Array{
...
var nclobWriteLocator = common.B1Array{
