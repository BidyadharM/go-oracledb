...
func TestHandleServerToClientPiggyback_NotFunction(t *testing.T) {
	...
	oracleErr, ok := err.(common.SQLError)
	if !ok {
		t.Errorf("Expected oracle error but got standard other error type")
	}

	if oracleErr.ErrorCode() != string(common.SPFNotFunction) {
		t.Errorf("Expected error to be UnknownSPFFunction: %s, but was %s", common.SPFNotFunction, oracleErr.ErrorCode())
	}
}
...
func TestHandleServerToClientPiggyback_UnknownFunction(t *testing.T) {
	...
	if oracleErr.ErrorCode() != string(common.UnknownSPFFunction) {
		t.Errorf("Expected error to be UnknownSPFFunction: %s, but was %s", common.UnknownSPFFunction, oracleErr.ErrorCode())
	}
}
