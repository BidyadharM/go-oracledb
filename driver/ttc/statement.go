...
func (s *Statement) QueryContext(ctx context.Context, args []driver.NamedValue) (driver.Rows, error) {
	...
	msgIn, _ := s.shelf.GetMessageStreamer().Drain(ctx, common.IN)
	...
	if msgIn != 0 {
		...
		return nil, s.shelf.LocalizeError(common.NewOracleError(common.InternalError, nil))
	}
...
func (s *Statement) _closeCursor() error {
	...
	occaMsg := msg.(*tTIOcca)
	occaMsg.setCursorIDs([]common.UB4{common.UB4(s.qualifiedQuery.cursorId)})
...
func (s *Statement) Close() error {
	...
	if s._rows != nil {
		...
		if err != nil {
			...
			finalErr = s.shelf.LocalizeError(common.NewOracleError(common.RowsCloseFailed, err)).(common.SQLError)
		}
		s._rows = nil
	}
	// close cursors
	...
	if err != nil {
		...
		finalErr = s.shelf.LocalizeError(common.NewOracleError(common.StatementCloseFailed, err)).(common.SQLError)
	}
