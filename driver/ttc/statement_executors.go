...
import (
	...
	"github.com/oracle/go-oracledb/driver/refcursor"
	"github.com/oracle/go-oracledb/driver/ttc/converters"
)

// Internal AL8I4 flags used in oall8Options[9]
const (
	_al8exGetPidmlrc        common.UB4 = 0x4000
	_al8exImplResultsClient common.UB4 = 0x8000
)
...
type statementProcessor struct {
	shelf         *ttiShelf[common.MessageType] // Message shelf used to get factory/streamer/marshaller/codec-factory for statement execution.
	sessCtx       *common.SessionContext        // The session context used to get the session character set
...
func (e *statementProcessor) SetShelf(s *ttiShelf[common.MessageType]) { e.shelf = s }

// SetSessionContext injects the session context used by the executor.
func (e *statementProcessor) SetSessionContext(sessCtx *common.SessionContext) { e.sessCtx = sessCtx }
...
func (m selectResultMetadata) columnCount() common.UB4 {
	return common.UB4(len(m.columns))
}
...
func (m selectResultMetadata) newRows(shelf *ttiShelf[common.MessageType]) *ttcRows {
	if len(m.columns) == 0 {
		return nil
	}
...
type statementExecutorExec struct {
	statementProcessor
	_currentRank        common.UB4      // currentRank represents current row in processing, incremented at each execution
...
func newStatementExecutorDML() *statementExecutorDML {
	return &statementExecutorDML{
		statementExecutorExec: statementExecutorExec{
			statementProcessor: statementProcessor{
				opts:  common.UB4(statementParsedByServer | executeStatement | commitAfterExecution | noPLSQLMode),
...
func newStatementExecutorPlSql() *statementExecutorPlSql {
	return &statementExecutorPlSql{
		statementExecutorExec: statementExecutorExec{
			statementProcessor: statementProcessor{
				opts:  common.UB4(statementParsedByServer | executeStatement | commitAfterExecution),
				al8i4: buildAl8i4(1, false, _al8exImplResultsClient, 1),
			},
			_currentRank: 1,
		},
...
func isNoDataFoundError(err error) bool {
	...
	return ok && sqlErr.ErrorCode() == string(common.NoDataFound)
}
...
func (e *statementExecutorSelect) QueryContext(ctx context.Context, query *qualifiedSQLStatement, namedValues []driver.NamedValue) (sqldriver.Rows, error) {
	...
	var messageToBeExecuted common.Message[common.MessageType]
	if e.needToSendOACs() {
...
*/
func (e *statementExecutorSelect) buildOAll8Options(isDefine bool) common.UB4 {
	if isDefine {
...
*/
func (e *statementExecutorSelect) prepareDefines(messageToBeExecuted common.Message[common.MessageType]) {
	defines := make([]common.Marshallable, len(e.resultMetadata.columns))
	...
	messageToBeExecuted.(*tTIOall).setDefCols(common.SB4(len(defines)))
...
*/
func (e *statementProcessor) createOAll8Msg(qualifiedStmt *qualifiedSQLStatement, args []sqldriver.Value) (common.Message[common.MessageType], error) {
	...
	if err != nil {
		...
		return nil, common.NewOracleError(common.StatementExecutionFailed, err, select_.String(), "QueryContext failed")
	}
	m, _ := reqMsg.(*tTIOall)
	m.setOptions(e.opts)
	...
	if len(args) > 0 {
		m.setNumberOfBindPositions(common.SB4(len(e.bindValues)))
		if e.needToSendOACs() {
...
*/
func (e *statementExecutorSelect) createOexfenMsg(query *qualifiedSQLStatement) (common.Message[common.MessageType], error) {
...
*/
func buildAl8i4(iterations common.UB4, selectStmt bool, flags common.UB4, parseOption common.UB4) []common.UB4 {
...
*/
func (e *statementProcessor) getMaxLengthForOac(position int, currLength int) common.UB4 {
	if e.previousOacs == nil || len(e.previousOacs) <= position {
		return common.UB4(currLength)
	}
	return max(e.previousOacs[position].(*tTIoac).maxLength, common.UB4(currLength))
}
...
func (e *statementExecutorSelect) runQuery(ctx context.Context, message common.Message[common.MessageType]) (*queryRunState, common.SB4, error) {
	...
	if err = stmr.Push(ctx, message); err != nil {
		...
		return nil, -1, common.NewOracleError(common.RunQueryError, err, "push")
	}
	...
	if len(e.encodedValues) > 0 && (e.opts&defineColumnsProvided == 0) {
		if err = e.pushBindRows(ctx, "runQuery", common.RunQueryError); err != nil {
			return nil, -1, err
		}
	}
	// flush Streamer
	...
	if err = stmr.Flush(ctx); err != nil {
		...
		return nil, -1, common.NewOracleError(common.RunQueryError, err, "flush")
	}
	...
	oerFound := false
	var returnedCursorID common.SB4
	var msg common.Message[common.MessageType]
	for {
		...
		if err != nil {
			...
			if err != nil {
				// Return error
				return nil, -1, common.NewOracleError(common.RunQueryError, err, "pull")
			}
		}
		...
		case TTIOER:
			...
			if err == nil && state.rows == nil {
				...
				return nil, -1, common.NewOracleError(common.ProtocolViolation, nil)
			}
			oerFound = true
		default:
			common.Odl.Warn("Unexpected msg received", "message", msg)
			return nil, -1, common.NewOracleError(common.InternalError, nil)
		}
		if oerFound {
...
func (e *statementExecutorPlSql) createRXD(t *messageHeader) (common.Message[common.MessageType], error) {
	...
	if err != nil {
		...
		return nil, common.NewOracleError(common.CallbackFactoryError, err, "createRXD failed")
	}
	rxd := msg.(*tTIrxd)
...
//   - the cursorID
//   - the error if any
func (e *statementExecutorExec) runExec(ctx context.Context, message common.Message[common.MessageType]) (sqldriver.Result, common.SB4, error) {
	...
	if err := stmr.Push(ctx, message); err != nil {
		...
		return nil, -1, common.NewOracleError(common.RunExecError, err, "push")
	}
	// If binds exist, encode and push RXD rows before flushing
	if len(e.encodedValues) > 0 {
		if err := e.pushBindRows(ctx, "runExec", common.RunExecError); err != nil {
			return nil, -1, err
		}
	}
	...
	if err := stmr.Flush(ctx); err != nil {
		...
		return nil, -1, common.NewOracleError(common.RunExecError, err, "flush")
	}
	registerRunExecCallbacks(stmr, e.shelf)
	defer unregisterRunExecCallbacks(stmr)

	oerFound := false
	var receivedCursorID common.SB4
	for {
		...
		if err != nil {
			...
			if err != nil {
				// Return error
				return nil, -1, common.NewOracleError(common.RunExecError, err, "pull")
			}
		}
		...
		case TTIOER:
			...
			oerFound = true
			e.opts &^= common.UB4(statementParsedByServer)
		case TTIRPA:
		...
		case TTIFOB:
			if err := stmr.Push(ctx, msg); err != nil {
				...
				return nil, -1, common.NewOracleError(common.RunExecError, err, "ttifob-push")
			}
			if err := stmr.Flush(ctx); err != nil {
				common.Odl.Error("StatementExecutorDML.ExecContext: TTIFOB flush failed",
					"error", err, "stage", "ttifob-flush")
				return nil, -1, common.NewOracleError(common.RunExecError, err, "ttifob-flush")
			}
		default:
			common.Odl.Warn("Unexpected msg received", "message", msg)
			return nil, -1, common.NewOracleError(common.InternalError, nil)
...
func (e *statementProcessor) handleContextCancelled(ctx context.Context) (common.Message[common.MessageType], error) {
...
func (e *statementExecutorSelect) handleDCB(state *queryRunState, msg common.Message[common.MessageType]) error {
	dcb := msg.(*tTIdcb)
	...
	if err != nil {
		...
		return common.NewOracleError(common.RunQueryError, err, "column-meta")
	}
...
func (e *statementExecutorSelect) createBVC(t *messageHeader) (common.Message[common.MessageType], error) {
	...
	if err != nil {
		...
		return nil, common.NewOracleError(common.CallbackFactoryError, err, "createBVC failed")
	}
	bvc := msg.(*tTIbvc)
...
func (s *queryRunState) handleBVC(msg common.Message[common.MessageType]) {
	bvc := msg.(*tTIbvc)
...
func (e *statementExecutorSelect) createRXD(state *queryRunState, t *messageHeader) (common.Message[common.MessageType], error) {
	msg, err := e.shelf.GetMessageFactory().(Factory).GetMessage(TTIRXD)
	// map factory failure -> OGD-00073 CallbackFactoryError("get-rxd")
	if err != nil {
		common.Odl.Error("createRXD: GetMessage(TTIRXD) failed", "error", err, "stage", "get-rxd")
...
*/
func (e *statementExecutorDML) createRXD(t *messageHeader) (common.Message[common.MessageType], error) {
	msg, err := e.shelf.GetMessageFactory().(Factory).GetMessage(TTIRXD)
	// map factory failure -> OGD-00073 CallbackFactoryError("get-rxd")
	if err != nil {
		common.Odl.Error("createRXD: GetMessage(TTIRXD) failed", "error", err, "stage", "get-rxd")
...
*/
func (e *statementExecutorExec) handleRXDRow(msg common.Message[common.MessageType]) error {
...
func (e *statementExecutorDML) registerDMLCallbacks(ctx context.Context) {
	...
	stmr.RegisterPostUnmarshallCallback(TTIRPA, func(msg common.Message[common.MessageType], prevErr error) (bool, error) {
		if prevErr != nil {
		...
		if rpa, ok := msg.(*ttioallrpa); ok {
			...
			if err := rpa.UnMarshalDMLRows(ctx, e.shelf.GetMarshaller()); err != nil {
				...
				return false, common.NewOracleError(common.RunExecError, err, "ExecContext failed")
			}
		}
		return true, nil
	})
...
func (e *statementExecutorExec) registerIOVCallbacks(ctx context.Context) {
	stmr := e.shelf.GetMessageStreamer().(MessageStreamerInterface)
	stmr.RegisterPreUnmarshallCallback(TTIIOV, func(t *messageHeader) (common.Message[common.MessageType], error) {
	...
	dropUnhandled := func(msg common.Message[common.MessageType], prevErr error) (bool, error) {
		if prevErr != nil {
			return false, prevErr
		}
		return false, nil
...
func (s *queryRunState) handleRXDRow(msg common.Message[common.MessageType]) {
	if rxd, ok := msg.(*tTIrxd); ok && rxd != nil {
		currRow := make([]common.B1Array, len(rxd.row))
		for i := range rxd.row {
			currRow[i] = append(common.B1Array(nil), rxd.row[i]...)
		}
...
func (e *statementExecutorSelect) registerRunQueryCallbacks(state *queryRunState) {
	...
	stmr := e.shelf.GetMessageStreamer().(MessageStreamerInterface)
	stmr.RegisterPreUnmarshallCallback(TTIRXD, func(t *messageHeader) (common.Message[common.MessageType], error) {
...
func registerRunExecCallbacks(stmr MessageStreamerInterface, shelf *ttiShelf[common.MessageType]) {
...
func registerOallRpaCallbacks(stmr MessageStreamerInterface, shelf *ttiShelf[common.MessageType]) {
	...
	stmr.RegisterPreUnmarshallCallback(TTIRPA, func(t *messageHeader) (common.Message[common.MessageType], error) {
		...
		if err != nil {
			...
			return nil, common.NewOracleError(common.CallbackFactoryError, err, "registerOallRpaCallback failed")
		}
		return msg, nil
	})
}
...
func registerDMLOallRpaCallbacks(ctx context.Context, stmr MessageStreamerInterface, shelf *ttiShelf[common.MessageType]) {
	common.Odl.Debug("registerDMLOallRpaCallbacks: registering II-RPA callback")
	stmr.RegisterPostUnmarshallCallback(TTIRPA, func(msg common.Message[common.MessageType], prevErr error) (bool, error) {
		if prevErr != nil {
			return false, prevErr
...
func (s statementExecutorOperationNotSupported) ExecContext(_ context.Context, _ *qualifiedSQLStatement, _ []driver.NamedValue) (driver.Result, error) {
	...
	return nil, common.NewOracleError(common.StatementExecutionFailed, nil,
		s.kind.String(), "Exec")
}
