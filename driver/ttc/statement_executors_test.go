...
func TestStatementExecutor_Select_SuccessOERWithoutDCB(t *testing.T) {
	...
	if sqlErr.ErrorCode() != string(common.ProtocolViolation) {
		t.Fatalf("expected %s, got %s: %v", common.ProtocolViolation, sqlErr.ErrorCode(), err)
	}
}
...
func TestStatementExecutor_Select_Callback_GetMessage_RXD_Error_Integration(t *testing.T) {
	...
	if _, err := exec.QueryContext(ctx, q, nil); err == nil {
		...
		if sqlError.ErrorCode() != string(common.RunQueryError) {
			t.Fatalf("Expected error %s, but cas %s", common.RunQueryError, sqlError.ErrorCode())
		}
	}
}
...
func TestStatementExecutor_Select_DoesNotReuseStaleBVCStateAcrossExecutions(t *testing.T) {
	...
	if queryErr != nil {
		if sqlErr, ok := queryErr.(common.SQLError); !ok || sqlErr.ErrorCode() != string(common.NoDataFound) {
