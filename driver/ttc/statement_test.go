		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr && err != nil {
				if sqlError, ok := err.(common.SQLError); ok {
					if sqlError.ErrorCode() != string(common.CancelOperationError) {
						t.Errorf("Wrong error code, expected %s but was %s", string(common.CancelOperationError), sqlError.ErrorCode())
					}
					cause := sqlError.Unwrap()
					if cause == nil {
...
func TestStatementQueryContextLocalization(t *testing.T) {
	...
	if sqle, ok := err.(common.SQLError); !ok {
		t.Fatalf("expected SQLError, got %T", err)
	} else if sqle.ErrorCode() != string(common.InternalError) {
		t.Fatalf("expected error code %s, got %s", common.InternalError, sqle.ErrorCode())
	}
...
func runSeededStatementQuery(t *testing.T, query string, args []driver.NamedValue) {
	...
	stmt, err := newStatement(shelf, &common.SessionContext{}, query)
	if err != nil {
