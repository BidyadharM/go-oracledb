...
func (t *transaction) Commit() error {
	...
	msgIn, _ := t._underlyingConnection.shelf.GetMessageStreamer().Drain(ctx, common.IN)
	...
	if msgIn != 0 {
		...
		return t._underlyingConnection.shelf.LocalizeError(common.NewOracleError(common.InternalError, nil))
	}

	if readFuncError != nil {
		return t._underlyingConnection.shelf.LocalizeError(common.NewOracleError(common.ErrorInTransaction, readFuncError, "Commit"))
	}
...
func (t *transaction) Rollback() error {
	...
	msgIn, _ := t._underlyingConnection.shelf.GetMessageStreamer().Drain(common.BackgroundContext, common.IN)
	// no message should be left at this point
	if msgIn != 0 {
		// should drop connection.
		common.Odl.Error("unexpected messages remained after query; invalidating connection",
	...
	if runFuncErr != nil {
		return t._underlyingConnection.shelf.LocalizeError(common.NewOracleError(common.ErrorInTransaction, runFuncErr, "Rollback"))
	}
...
func newNotInTransactionError() error {
	return common.NewOracleError(common.NotInTransaction, nil, nil)
}
