...
func (kvl KeyValueList) String() string {
	...
	for kv := kvl.Front(); kv != nil; kv = kv.Next() {
		res.WriteString(kv.Value.(*common.KeyValue).String())
		res.WriteString(";")
	}
	res.WriteString(")")
	return res.String()
...
func (kvl KeyValueList) Equals(okvl *KeyValueList) bool {
	...
	for e := kvl.Front(); e != nil; e = e.Next() {
		if !e.Value.(*common.KeyValue).Equals(okv.Value.(*common.KeyValue)) {
			return false
		}
		okv = okv.Next()
	}
...
func validateAuthKeyValueLength(field string, length common.SB4, limit int) error {
	if length < 0 {
		common.Odl.Debug("invalid key/value length", "field", field, "length", length, "limit", limit)
		return common.NewOracleError(common.FailUnmarshal, nil, "Key/Value")
	}
	if length > common.SB4(limit) {
...
func (keyValueList *KeyValueList) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	var size int
	for kv := keyValueList.Front(); kv != nil; kv = kv.Next() {
		size = len(kv.Value.(*common.KeyValue).Key)
		//marshal value size
		err := engine.MarshalUB4(ctx, common.UB4(size))
		if err != nil {
			return _wrapError(err, "marshal Key/Value")
		}
		if size > 0 {
			//marshal value
			err = engine.MarshalCLR(ctx, kv.Value.(*common.KeyValue).Key, 0, size)
			if err != nil {
				return _wrapError(err, "marshal Key/Value")
			}
		}

		size = len(kv.Value.(*common.KeyValue).Value)
		//marshal key size
		err = engine.MarshalUB4(ctx, common.UB4(size))
		if err != nil {
			return _wrapError(err, "marshal Key/Value")
		...
		if size > 0 {
			//marshal key
			err = engine.MarshalCLR(ctx, kv.Value.(*common.KeyValue).Value, 0, size)
			if err != nil {
				return _wrapError(err, "marshal Key/Value")
			}
		}

		err = engine.MarshalSB4(ctx, kv.Value.(*common.KeyValue).Flag)
		if err != nil {
			return _wrapError(err, "marshal Key/Value")
		}
	}
...
func (keyValueList *KeyValueList) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
...
const (
	maxKeywordValueArrayPairs       common.UB4 = 128 * 10
	maxKeywordValueArrayValueLength common.UB4 = 64 * 1024 * 10
)

type keywordValueArray []keywordValuePair

func newKeywordValueArray(pairCount common.UB4) (*keywordValueArray, error) {
	if pairCount > maxKeywordValueArrayPairs {
		...
		return nil, common.NewOracleError(common.FailUnmarshal, nil, "keyword/value")
	}
...
func (keywordValueList keywordValueArray) MarshalTo(ctx context.Context, engine common.Marshaller) error {
...
func (keywordValueList keywordValueArray) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
	if common.UB4(len(keywordValueList)) > maxKeywordValueArrayPairs {
		common.Odl.Debug("keyword/value pair count exceeds maximum",
			"pairs", len(keywordValueList), "limit", maxKeywordValueArrayPairs)
		return common.NewOracleError(common.FailUnmarshal, nil, "keyword/value")
	}
	for i := 0; i < len(keywordValueList); i++ {
...
func (dalc *dynamicAllocatedArray) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	length := len(dalc.value)
	err := engine.MarshalUB4(ctx, common.UB4(length))
	if err != nil {
...
func (dalc *dynamicAllocatedArray) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
	...
	if length > 0 {
		value := make(common.B1Array, length)
...
func GetKeyValueFromKeyword(nlsKeys [64]string, keyword keywordValuePair) (string, any) {
	...
	if int(intValue) < len(nlsKeys) {
		...
		if len(textValue.value) > 0 {
			roleNames := strings.TrimSpace(common.B1ArrayToString(textValue.value))
