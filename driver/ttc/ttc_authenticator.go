...
type PasswordAuthenticator struct {
	...
	_logonMode                     int64
	_shelf                         ttiShelf[common.MessageType]
	_serverCompileTimeVerifierType byte
	_sessionContext                *common.SessionContext
...
func NewPasswordAuthenticator(username, password string, connectString string) *PasswordAuthenticator {
	pa := PasswordAuthenticator{
		...
		_username:      common.StringToB1Array(sanitizeInputCredential(username)),
		_password:      common.StringToB1Array(sanitizeInputCredential(password)),
		_logonMode:     0,
	}
	return &pa
}
...
func (pa *PasswordAuthenticator) validatePasswordLength() error {
	if len(pa._password) > maxPasswordBytes {
		return common.NewOracleError(common.AuthenticatorError, nil, "password length")
	}
	return nil
}
...
func (pa *PasswordAuthenticator) _doOSESSKEY(ctx context.Context) error {
	...
	osesskeyRPACallBack := func(t *messageHeader) (common.Message[common.MessageType], error) {
		...
		if err != nil {
			...
			return nil, common.NewOracleError(common.AuthenticatorError, err, nil)
		}
		return msg, nil
	}
	...
	for {
		...
		default:
			...
			return common.NewOracleError(common.InternalError, err, nil)
	...
	msgIn, _ := streamer.Drain(ctx, common.IN)
	// no message should be left at this point
	if msgIn != 0 {
		...
		return shelf.LocalizeError(common.NewOracleError(common.InternalError, nil))
	}
...
func (pa *PasswordAuthenticator) _doOAuth(ctx context.Context) error {
	...
	oauthMsg.(*oAuth).setEncryptedSK(sessionProperties.GetProperty(authSesskey).(*common.KeyValue).Value)
	oauthMsg.(*oAuth).setSalt(sessionProperties.GetProperty(authVFRData).(*common.KeyValue).Value)

	err = oauthMsg.(*oAuth).setVerifierType(int(sessionProperties.GetProperty(authVFRData).(*common.KeyValue).Flag))
	if err != nil {
	...
	if !isValidPBKDF2VgenCount(PBKDF2VgenCount) {
		...
		return common.NewOracleError(common.AuthenticatorError, nil, "invalid PBKDF2VgenCount")
	}
	...
	if !isValidPBKDF2SderCount(PBKDF2SderCount) {
		...
		return common.NewOracleError(common.AuthenticatorError, nil, "invalid PBKDF2SderCount")
	}

	oauthMsg.(*oAuth).setPBKDF2Salt(sessionProperties.GetProperty(authPDKDF2CSKSalt).(*common.KeyValue).Value)
	...
	serverResponse := common.B1ArrayToString(pa._sessionContext.GetSessionProperties().
		GetProperty(authServerResponse).(common.B1Array))
