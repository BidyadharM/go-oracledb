...
func newAuthTestShelf(bufSize int) (*ttiShelf[common.MessageType], *MessageStreamer, *ArrayBasedDataBuffer) {
	...
	_ = funcReg.Register(functionRegistryKey{messageType: TTIFUN, functionType: oSesskey}, -1, func() common.Message[common.MessageType] {
		return NewOSesskey()
	})
	// Use NewOAuth18 to mirror production behaviour where the key/value list is pre-initialized.
	_ = funcReg.Register(functionRegistryKey{messageType: TTIFUN, functionType: oauth}, -1, func() common.Message[common.MessageType] {
		return NewOAuth18()
	})
	// RPA responses used by the authenticator Pull paths
	_ = funcReg.Register(functionRegistryKey{messageType: TTIRPA, functionType: oSesskey}, -1, func() common.Message[common.MessageType] {
		return NewOSesskeyRPA()
	})
	_ = funcReg.Register(functionRegistryKey{messageType: TTIRPA, functionType: oauth}, -1, func() common.Message[common.MessageType] {
		return NewOAuthRPA()
	})
	_ = funcReg.Register(functionRegistryKey{messageType: TTISPF, functionType: common.FunctionType(ocssync)}, -1, NewttiSPFOCSSync)
...
func TestPasswordAuthenticator_doOSESSKEY_Golden(t *testing.T) {
	...
	pa.SetShelf(shelf)
	pa.SetSessionContext(common.NewSessionContext())
	...
	if v, _ := strconv.Atoi(string(sessionProperties.GetProperty(authPbkdf2VgenCount).(*common.KeyValue).Value)); v != 4096 {
		t.Errorf("PBKDF2VgenCount = %d, want 4096", v)
	}
	if sder, _ := strconv.Atoi(common.B1ArrayToString(sessionProperties.GetProperty(authPbkdf2SderCount).(*common.KeyValue).Value)); sder != 3 {
...
func TestPasswordAuthenticator_doOAuth_Golden(t *testing.T) {
	...
	unmarshallable, _ := osrpaMsg.(common.UnMarshallable)
	...
	if err := pa._doOAuth(ctx); err != nil {
		sqlError, ok := err.(common.SQLError)
		if !ok {
			t.Fatalf("expected error to be SQLError")
		}
		if sqlError.ErrorCode() != string(common.AuthenticatorError) {
			t.Fatalf("Expected error %s, but got %s", common.AuthenticatorError, sqlError.ErrorCode())
		}
	}
}

func writeAuthWarning(t *testing.T, ctx context.Context, buf *ArrayBasedDataBuffer, number common.UB2, message string, flags common.UB2) {
	t.Helper()
	...
	if err := mar.MarshalUB2(ctx, common.UB2(len(message))); err != nil {
