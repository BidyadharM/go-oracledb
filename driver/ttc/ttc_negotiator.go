...
func (cn *ConnectionNegotiator) Negotiate(ctx context.Context) (*common.SessionContext, *ttiShelf[common.MessageType], error) {
	var pro *tTIpro
	var dty *tTIdty
...
func (cn *ConnectionNegotiator) SetDataBuffer(buf common.DataBuffer) {
	cn._dataBuffer = buf
}
...
func (cn *ConnectionNegotiator) _negotiateDatatype(
	ctx context.Context,
	msgStmr common.Streamer[common.MessageType],
	msgfactory common.Factory,
	pro *tTIpro,
) (*tTIdty, error) {
	...
	if err != nil {
		...
		return nil, common.NewOracleError(common.NegotiatorError, err, nil)
	...
	dtyCallBack := func(header *messageHeader) (common.Message[common.MessageType], error) {
	...
	if err != nil {
		...
		return nil, common.NewOracleError(common.NegotiatorError, err)
	}
...
func (cn *ConnectionNegotiator) _negotiateProtocol(
	...
	err = msgStmr.Push(ctx, proMsg.(common.Message[common.MessageType]))
	if err != nil {
	...
	default:
		...
		return nil, common.NewOracleError(common.InternalError, err, nil)
	}
}

// _createShelfAndSessionContext manages creation of Shelf and SessionContext.
func _createShelfAndSessionContext() (*ttiShelf[common.MessageType], *common.SessionContext) {
...
func _createAndRegisterMessageFactory(
	shelf *ttiShelf[common.MessageType], version int8) common.Factory {
...
func _createAndRegisterCodecFactory(
	shelf *ttiShelf[common.MessageType], version int8) {
...
func _createAndRegisterMarshaller(
	buf common.DataBuffer,
	shelf *ttiShelf[common.MessageType],
	isNative bool,
) {
	var mar common.Marshaller
	if isNative {
	...
	if props := shelf.GetConnectionProperties(); props != nil {
		if marshalEngine, ok := mar.(*MarshalEngine); ok {
			marshalEngine.setDefaultLobPrefetchSize(int64(props.DefaultLobPrefetchSize))
		}
	}
