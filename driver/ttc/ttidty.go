...
const (
	al32Utf8CharSet   common.UB2 = 873
	al16Utf16CharSet  common.UB2 = 2000
	dbcsCharSet       common.UB2 = 0xffff // -1
	ucs2CharSet       common.UB2 = 0xfbff // -5
	al24UtffssCharSet common.UB2 = 870
	utf8CharSet       common.UB2 = 871
)
...
func NewTTIdty() common.Message[common.MessageType] {
	obj := &tTIdty{
...
func (p *tTIdty) GetMsgCode() common.MessageType {
	return TTIDTY
}

// MarshalTo serializes the tTIdty structure into the marshal engine.
func (p *tTIdty) MarshalTo(ctx context.Context, mar common.Marshaller) error {
	var err error
	...
	if err = mar.MarshalUB2(ctx, common.UB2(cliRINLE)); err != nil {
	...
	if err = mar.MarshalUB2(ctx, common.UB2(cliROUTLE)); err != nil {
		common.Odl.Warn("Failed to marshal message", "error", err)
		return common.NewOracleError(common.FailMarshal, err, TTCMsgTypeDescription[p.GetMsgCode()])
	}

	// marshal cliFlags (protocol flags) field
	if err = mar.MarshalUB1(ctx, common.UB1(typeRepresentationTable.getFlags())); err != nil {
...
func (p *tTIdty) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	var err error
...
func isCharSetMultibyte(charSet common.UB2) bool {
	switch charSet {
