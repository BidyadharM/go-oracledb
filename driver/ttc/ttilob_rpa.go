...
func (p *ttiLobRpa) GetMsgCode() common.MessageType {
	return TTIRPA
}
...
func newTTILobRPA() common.Message[common.MessageType] {
	return &ttiLobRpa{}
}
...
func (p *ttiLobRpa) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	...
	if sourceLocator != nil && sourceLocator.hasBytes() {
		...
		if p.lobDefinition.operation == kplobTmpCreate {
			...
			if len(sourceLocatorBytes) != expectedLen {
				...
				sourceLocatorBytes = make(common.B1Array, expectedLen)
			}
...
func (p *ttiLobRpa) SetDefinition(lobDef *lobDefinition) error {
	if lobDef == nil {
		...
		return common.NewOracleError(common.InvalidLOBBuffer, nil, "marshal", TTCMsgTypeDescription[p.GetMsgCode()], "definition missing")
	}
