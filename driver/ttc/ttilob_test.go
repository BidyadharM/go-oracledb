...
func TestTTIlob_SetDefinition_NilDefinition(t *testing.T) {
	...
	if got, want := sqlErr.ErrorCode(), string(common.InvalidLOBBuffer); got != want {
...
func TestTTIlob_MarshalTo_Success(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			def: func() *lobDefinition {
				return &lobDefinition{
					sourceLocator:      newLocator(common.B1Array{0x01, 0x02, 0x03, 0x04}, common.UB8(4)),
					destinationLocator: newLocator(common.B1Array{0x05, 0x06, 0x07}, common.UB8(8)),
					charsetID:          common.UB2(873),
					nullO2U:            true,
					sendLobAmt:         true,
					lobAmt:             common.UB8(99),
					lobscnl:            2,
					lobscn:             []common.UB4{0x0, 0x1},
...
func TestTTIlob_MarshalTo_Fail(t *testing.T) {
	...
	makeCommonDefinition := func() *lobDefinition {
		return &lobDefinition{
			sourceLocator:      newLocator(common.B1Array{0x01, 0x02, 0x03, 0x04}, 0),
			destinationLocator: newLocator(common.B1Array{0x05, 0x06, 0x07, 0x08}, 0),
			charsetID:          common.UB2(873),
			sendLobAmt:         true,
			lobAmt:             common.UB8(1),
			lobscnl:            1,
			lobscn:             []common.UB4{0x01},
			operation:          kplobWrite,
		}
	}
