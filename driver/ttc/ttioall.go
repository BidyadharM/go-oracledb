...
type tTIOall struct {
	headerMarshaller common.Marshallable // headerMarshaller marshals the function code w/wo seq and token numbers
	sql              common.B1Array      // sql represents SQL text payload encoded in network character set
	options          common.UB4          // options represents operation flags (statementParsedByServer / bindValuesPresent / executeStatement / defineColumnsProvided /...)
	oall8Options     []common.UB4        // oall8Options represents variable-length vector (we use len=13)
	cursorId         common.SB4          // cursorId id of the cursorId
	maxLength        common.UB4          // maxLength represents max bytes for marshalling
	numberOfBinds    common.SB4          // numberOfBinds represents number of bindValuesPresent positions present in this request
	defineColumns    common.SB4          // defineColumns represents number of defineColumnsProvided columns provided by the client
	currentRank      common.UB4          // currentRank represents current row in processing, incremented at each execution
	maxClientBuffer  common.UB4          // maxClientBuffer represents max client buffer size for prefetch.
	rowsToFetch      common.UB4          // rowsToFetch represents number of rows to prefetch when SELECT

	// Phase 1: per-bind OAC definitions to be marshalled in OALL8; bind values are sent via TTIRXD.
	bindOACs []common.Marshallable

	// Phase 1: per-define OAC definitions to be marshalled in OALL8 for result-set columns.
	defineOACs []common.Marshallable
}

// NewOall18 constructs a new OALL8 TTC function message with default limits.
func NewOall18() common.Message[common.MessageType] {
	return &tTIOall{
...
func NewOall() common.Message[common.MessageType] {
	return &tTIOall{
...
func (m *tTIOall) GetMsgCode() common.MessageType { return TTIFUN }

// getFuncCode returns the TTC function code associated with OALL8.
func (m *tTIOall) GetFuncCode() common.FunctionType { return oAll8 }

// setSQL sets the SQL text payload (as network charset bytes).
func (m *tTIOall) setSQL(sql common.B1Array) { m.sql = sql }

// setOptions sets operation flags for OALL8.
func (m *tTIOall) setOptions(opts common.UB4) { m.options = opts }

// setOall8Options sets the i4 vector (length, iterations, flags, etc.).
func (m *tTIOall) setOall8Options(v []common.UB4) { m.oall8Options = v }

// setCursorId sets the cursor id.
func (m *tTIOall) setCursorId(c common.SB4) { m.cursorId = c }

// setMaxLength sets the maximum length used for marshalling.
func (m *tTIOall) setMaxLength(n common.UB4) { m.maxLength = n }
...
func (m *tTIOall) setNumberOfBindPositions(n common.SB4) { m.numberOfBinds = n }

// setDefCols sets the number of define columns provided by the client.
func (m *tTIOall) setDefCols(n common.SB4) { m.defineColumns = n }

// setCurrentRank sets the DML currentRank value used in TTC >=1200 branch.
func (m *tTIOall) setCurrentRank(r common.UB4) { m.currentRank = r }
...
*/
func (m *tTIOall) setBindOACs(currRowOac []common.Marshallable) { m.bindOACs = currRowOac }

// setDefineOACs attaches per-define OAC (Oracle Access Descriptor) definitions to this OALL8 request.
func (m *tTIOall) setDefineOACs(oacs []common.Marshallable) { m.defineOACs = oacs }

// MarshalTo serializes the OALL8 function message.
func (m *tTIOall) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	...
	// sql pointer/length
	if err := m._marshalPtr(ctx, engine, common.SB4(len(m.sql)), "Oall8-sql-ptr"); err != nil {
		common.Odl.Warn("Error marshalling OALL8 sql ptr", "error", err)
		return common.NewOracleError(common.FailMarshal, err, nil)
	}
	if err := engine.MarshalSB4(ctx, common.SB4(len(m.sql))); err != nil {
	...
	if err := engine.MarshalSB4(ctx, common.SB4(len(m.oall8Options))); err != nil {
	...
	regidLb, regidMb := 0, 0
	if err := engine.MarshalUB4(ctx, common.UB4(regidLb)); err != nil {
	...
	if err := engine.MarshalUB4(ctx, common.UB4(regidMb)); err != nil {
		common.Odl.Warn("Error marshalling OALL8 regidMb", "error", err)
		return common.NewOracleError(common.FailMarshal, err, nil)
	}

	if err := m._marshalPtr(ctx, engine, common.SB4(m.currentRank), "1200 ptr1"); err != nil {
		return err
	}
	...
	if err := m._marshalPtr(ctx, engine, common.SB4(m.currentRank), "1200 ptr2"); err != nil {
		return err
	}
...
func (m *tTIOall) _marshalPtr(ctx context.Context, engine common.Marshaller, n common.SB4, fieldName string) error {
	if n > 0 {
		if err := engine.MarshalPTR(ctx); err != nil {
