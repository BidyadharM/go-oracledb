...
type oAuth struct {
	...
	enableTempLobRefCnt common.B1Array
	encryptedSK         []byte
	alterSession        common.B1Array
...
func NewOAuth() common.Message[common.MessageType] {
	t := &oAuth{
...
func NewOAuth18() common.Message[common.MessageType] {
	t := &oAuth{
...
func (o *oAuth) _oauthInit() {
	...
	if o.enableTempLobRefCntBool {
		o.enableTempLobRefCnt = common.StringToB1Array(strconv.Itoa(kolrugEnable))
	} else {
		o.enableTempLobRefCnt = common.StringToB1Array(strconv.Itoa(0))
	}
...
func (o *oAuth) setSessionFields() {
	...
	quotedNLSLanguage := common.EnquoteLiteral(nlslanguage)
	...
	quotedNLSTerritory := common.EnquoteLiteral(nlsterritory)
	...
	if alterNLSLanguage != "" || alterNLSTerritory != "" || o.isSessionTZ {
		quotedTimeZone := common.EnquoteLiteral(o.sessionTimeZone)
		...
		o.alterSession = common.StringToB1Array(doAlter)
...
func (o *oAuth) GetMsgCode() common.MessageType {
	return TTIFUN
}

func (o *oAuth) GetFuncCode() common.FunctionType {
	return oauth
}
...
func (o *oAuth) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	...
	if err != nil {
		...
		return common.NewOracleError(common.FailMarshal, err, TTCMsgTypeDescription[o.GetMsgCode()])
	}
	...
	if len(o.user) > 0 {
		...
		err = engine.MarshalSB4(ctx, common.SB4(len(o.user))) // authusrl
		if err != nil {
	...
	err = engine.MarshalUB4(ctx, common.UB4(o.logonMode)) // authflg
	if err != nil {
	...
	err = engine.MarshalUB4(ctx, common.UB4(o.keyValList.Len())) // authivln
	if err != nil {
	...
	err = ((common.Marshallable)(o.keyValList)).MarshalTo(ctx, engine)
	if err != nil {
		common.Odl.Warn("Error marshalling oauth", "error", err)
		return common.NewOracleError(common.FailMarshal, err, nil)
	}
...
// Marshal will write.
func (o *oAuth) prepareForOAUTH(luser common.B1Array,
	lpassword []byte,
...
// for instance.
func (o *oAuth) initializeLogonModeForOAUTH(luser common.B1Array, llogonMode int64, lpassword []byte) {
...
func (o *oAuth) setPasswordKeyValsForOAUTH(lpassword []byte, speedyKey []byte) {
	if lpassword != nil {
		o.keyValList.PushBack(&common.KeyValue{Key: _authPasswordKey, Value: lpassword})
	}

	if speedyKey != nil {
		o.keyValList.PushBack(&common.KeyValue{Key: _authPbkdf2SpeedyKey, Value: speedyKey})
	}
...
func (o *oAuth) setVSessionKeyValsForOAUTH() {
	...
	v := _keyValStaticInfoForOAuthConnectString.Value.(*common.KeyValue)
	v.Value = o.connectString
	if len(o.clientName) != 0 {
		o.keyValList.PushBack(&common.KeyValue{Key: _authProxyClientNameKey, Value: o.clientName})
	}
...
var _authOraEditionKey = common.StringToB1Array(authOraEdition)
var _authSessionClientDrvnmKey = common.StringToB1Array(authSessionClientDrvnm)
var _authSessionClientVsnKey = common.StringToB1Array(authSessionClientVsn)
...
func (o *oAuth) setDriverIdentityKeyValsForOAUTH() {
	if len(o.editionName) != 0 {
		o.keyValList.PushBack(&common.KeyValue{Key: _authOraEditionKey, Value: o.editionName})
	}

	o.keyValList.PushBack(&common.KeyValue{Key: _authSessionClientDrvnmKey, Value: currentDriverName})

	o.keyValList.PushBack(&common.KeyValue{Key: _authSessionClientVsnKey, Value: common.NumericDriverVersion})
}

var _authAlterSessionKey = common.StringToB1Array(authAlterSession)

func (o *oAuth) setAlterSessionKeyValsForOAUTH() {
	o.keyValList.PushBack(&common.KeyValue{Key: _authAlterSessionKey, Value: o.alterSession, Flag: 1})
}

func (o *oAuth) _initializeOAuthResponse(userPassword common.B1Array) error {
...
func (o *oAuth) setSalt(salt common.B1Array) {
	o.salt = salt
}

func (o *oAuth) setEncryptedSK(sk common.B1Array) {
	o.encryptedSK = sk
}
...
func (o *oAuth) setVerifierType(verifierType int) error {
	if verifierType != ZtvtOrcl7 &&
		...
		return common.NewOracleError(common.AuthenticatorError, nil, nil)
...
func (o *oAuth) setPBKDF2Salt(salt common.B1Array) {
	o.PBKDF2Salt = salt
}
...
func (o *oAuth) setUser(username common.B1Array) {
	o.user = username
}

func (o *oAuth) setConnectString(connectString string) {
	o.connectString = common.StringToB1Array(connectString)
}
...
type OAuthRPA struct {
	outNbPairs       common.UB2
	sessionTimeZone  string
	connectionValues *common.Properties[string]
}
...
func NewOAuthRPA() common.Message[common.MessageType] {
	return &OAuthRPA{}
}
...
func (rpa *OAuthRPA) GetMsgCode() common.MessageType {
	return TTIRPA
}
...
func (rpa *OAuthRPA) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
	...
	if err = validateAuthKeyValueLength("pairs", common.SB4(rpa.outNbPairs), maxAuthKeyValuePairs); err != nil {
		return err
	}

	//for o5logon, the flag is the verifier type
	keyValueList := NewPreallocatedKeyValueList(int(rpa.outNbPairs))
	err = ((common.UnMarshallable)(keyValueList)).UnMarshalFrom(ctx, engine)
	if err != nil {
		common.Odl.Warn("Unable to unmarshal oAuth RPA, cant' unmarshal key/value pairs", "error", err)
		return common.NewOracleError(common.FailUnmarshal, err, nil)
	}

	rpa.connectionValues = common.NewProperties[string]()
	for kv := keyValueList.Front(); kv != nil; kv = kv.Next() {
		rpa.connectionValues.SetProperty(
			strings.TrimSpace(common.B1ArrayToString(kv.Value.(*common.KeyValue).Key)),
			kv.Value.(*common.KeyValue).Value)
	}
	...
	if !rpa.connectionValues.ContainsKey(authDbMountID) {
		...
		return common.NewOracleError(common.FailUnmarshal, nil, nil)
	}
	...
	if authDBMountID != "" &&
		...
		return common.NewOracleError(common.DatabaseMountStateError, nil, nil)
	}
