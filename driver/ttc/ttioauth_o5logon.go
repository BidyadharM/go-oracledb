...
func (o *o5Logon) _buildO5LogonKey(verifier int, ka []byte, start1 int,
	...
	if o.useO7LMR {
		...
		common.BArray2Nibbles(kbka, kakbNibbles)
		saltBinary := common.ToBinArray(string(sha512SaltPBKDF2Nibbles))
...
func (o *o5Logon) generateOAuthResponse(
	verifierType int,
	salt []byte,
	noQuotesUser common.B1Array,
	noQuotesPwd common.B1Array, // password string
	noQuotesPwdNetBytes common.B1Array, // password represented in network char set
...
func (o *o5Logon) constructVerifierForSHA512(pk []byte, salt []byte) ([]byte, error) {
	...
	h.Write(pk)
	if salt != nil {
		h.Write(common.ToBinArray(string(salt)))
	}
...
func (o *o5Logon) constructVerifierExceptSHA512(verifierType int,
	...
	if verifierType == ZtvtSSH1 || verifierType == ZtvtSh1 {
		...
		if verifierType == ZtvtSSH1 && salt != nil {
			_, err = o.sha1Hash.Write(common.ToBinArray(string(salt)))
			if err != nil {
				return nil, err
			}
		}
		...
		if verifierType == ZtvtSmd5 {
			_, err = o.md5Hash.Write(common.ToBinArray(string(salt)))
			if err != nil {
				return nil, err
			}
		}
...
func (o *o5Logon) generatePk(PBKDF2VgenCount int, noQuotesPwd common.B1Array, salt []byte) ([]byte, error) {
	...
	saltForPBKDF2Part1 := common.ToBinArray(string(salt))
...
func (o *o5Logon) generateKb(ka []byte, verifier []byte,
	...
	kb := make([]byte, len(ka))
	if err := common.GenerateRandomBytes(kb); err != nil {
	...
	common.BArray2Nibbles(encryptedKBBytes, encryptedKBNibbles)
	return kb, nil
}
...
func (o *o5Logon) generateSpeedKey(confounderLen int, verifierType int,
	...
	if verifierType == ZtvtSha512 {
		if err := common.GenerateRandomBytes(confounder); err != nil {
		...
		pkEncryptedNibblesLength[0] = common.BArray2Nibbles(pkEncrypted, pkEncryptedNibbles)
	}
...
func (o *o5Logon) _encryptPassword(noQuotesPwdNetBytes []byte,
	...
	if encryptedPasswordNibbles == nil {
		return common.NewOracleError(common.AuthenticatorError, nil, "resource E missing")
	}
	...
	encryptedPasswordNibblesLength[0] = common.BArray2Nibbles(jenc, encryptedPasswordNibbles)
...
func validateNibbleBufferLength(array []byte, nibbles []byte) error {
	if len(nibbles) < len(array)*2 {
		return common.NewOracleError(common.AuthenticatorError, nil, "nibbles buffer too small")
	}
	return nil
}
