...
var _minimumPBKDF2VgenCountStr = common.B1Array(strconv.Itoa(minimumPBKDF2VgenCount))
...
func NewOSesskey() common.Message[common.MessageType] {
	obj := &oSessionKey{
...
func NewOSesskey18() common.Message[common.MessageType] {
	obj := &oSessionKey{
...
func (sess *oSessionKey) GetMsgCode() common.MessageType {
	return TTIFUN
}

func (sess *oSessionKey) GetFuncCode() common.FunctionType {
	return oSesskey
}
...
func (sess *oSessionKey) MarshalTo(ctx context.Context, engine common.Marshaller) error {
	...
	if err != nil {
		...
		return common.NewOracleError(common.FailMarshal, err, TTCMsgTypeDescription[sess.GetMsgCode()])
	}
	...
	if len(sess.user) > 0 {
		...
		err := engine.MarshalSB4(ctx, common.SB4(len(sess.user))) // authusrl
		if err != nil {
	...
	err = engine.MarshalUB4(ctx, common.UB4(sess.logonMode)) // authflg
	if err != nil {
	...
	err = engine.MarshalUB4(ctx, common.UB4(sess.keyValList.Len())) // authivln
	if err != nil {
...
func (sess *oSessionKey) setUser(user common.B1Array) {
	sess.user = user
}
...
func NewOSesskeyRPA() common.Message[common.MessageType] {
	return &oSesskeyRPA{}
}
...
func (rpa *oSesskeyRPA) GetMsgCode() common.MessageType {
	return TTIRPA
}
...
func (rpa *oSesskeyRPA) UnMarshalFrom(ctx context.Context, engine common.Marshaller) error {
	...
	if err = validateAuthKeyValueLength("pairs", common.SB4(outNbPairs), maxAuthKeyValuePairs); err != nil {
		return err
	}
	...
	for kv := keyValueList.Front(); kv != nil; kv = kv.Next() {
		rpa.connectionValues.SetProperty(
			common.B1ArrayToString(kv.Value.(*common.KeyValue).Key),
			kv.Value.(*common.KeyValue))
	}
	...
	PBKDF2Vgen, _ := rpa.connectionValues.GetProperty(authPbkdf2VgenCount).(*common.KeyValue)
	PBKDF2VgenCount, err := strconv.Atoi(common.B1ArrayToString(PBKDF2Vgen.Value))
	if err != nil {
		return err
	}
	if !isValidPBKDF2VgenCount(PBKDF2VgenCount) {
		...
		common.Odl.Warn(fmt.Sprintf("PBKDF2VgenCount outside allowed range [%d, %d] received: [%d]",
			...
			PBKDF2VgenCount))
		return common.NewOracleError(common.FailUnmarshal, nil, "invalid PBKDF2VgenCount")
	}

	PBKDF2Sder, _ := rpa.connectionValues.GetProperty(authPbkdf2SderCount).(*common.KeyValue)
	PBKDF2SderCount, err := strconv.Atoi(common.B1ArrayToString(PBKDF2Sder.Value))
	if err != nil {
	...
	if !isValidPBKDF2SderCount(PBKDF2SderCount) {
		...
		common.Odl.Warn(fmt.Sprintf("PBKDF2SderCount outside allowed range [%d, %d] received: [%d]",
			...
			PBKDF2SderCount))
		return common.NewOracleError(common.FailUnmarshal, nil, "invalid PBKDF2SderCount")
	}
