...
func newTTIrxd() common.Message[common.MessageType] {
	return &tTIrxd{}
}

// GetMsgCode implements Message.GetMsgCode, returning the TTC RXD message code.
func (rxd *tTIrxd) GetMsgCode() common.MessageType {
	return TTIRXD
}
...
func (rxd *tTIrxd) SetNumberOfColumns(noOfCols common.UB4) {
...
func (rxd *tTIrxd) SetRowCount(rowCount common.UB4) {
...
func (rxd *tTIrxd) SetPrevRow(row []common.B1Array) {
	rxd.prevRow = row
}
...
func (rxd *tTIrxd) SetBvcState(colSent *common.BitSet, found bool) {
...
func (rxd *tTIrxd) setSessionNCharacterSet(sessNCharSet common.UB2) {
	rxd.sessNCharSet = sessNCharSet
}

// SetSessionNCharacterSet sets session character set
func (rxd *tTIrxd) setSessionCharacterSet(sessCharSet common.UB2) {
...
func (rxd *tTIrxd) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	// DML returning case
	if rxd.numberOfReturningPositions > 0 && rxd.isDmlReturning {
		rxd.row = make([]common.B1Array, rxd.numberOfReturningPositions)
	...
	if rxd.numberOfReturningPositions > 0 {
		rxd.numberOfColumns = common.UB4(rxd.numberOfReturningPositions)
	}
		...
		for col := 0; col < int(rxd.numberOfColumns); col++ {
			if rxd.bvcColSent != nil && rxd.bvcColSent.Get(col) {
				...
				if rxd.prevRow[col] != nil {
					tmp := make(common.B1Array, len(rxd.prevRow[col]))
...
func (rxd *tTIrxd) _unmarshalScalarColumn(ctx context.Context, mar common.Marshaller, col int) error {
...
*/
func (rxd *tTIrxd) _unmarshalClobColumn(ctx context.Context, mar common.Marshaller, col int) error {
	// length
