...
func (t *RepresentationTable) MarshalTo(ctx context.Context, mar common.Marshaller) error {
	// send dty and rep as UB2
	for i := 1; i < int(t.representations[0]); i++ {
		if err := mar.MarshalUB2(ctx, common.UB2(t.representations[i])); err != nil { // LSB is false
			return err
		}
	}
...
func (t *RepresentationTable) UnMarshalFrom(ctx context.Context, mar common.Marshaller) error {
	var b common.UB2
	var err error
	...
	for {
		...
		if err != nil {
			...
			return common.NewOracleError(common.FailUnmarshal, err, "TypeRep")
		}
		...
		if !inTypeBlock {
			...
			if nbDty > _maxReceivedReps {
				...
				return common.NewOracleError(common.ProtocolViolationLimitExceeded, nil, "nbDty", _maxReceivedReps, nbDty)
			}
