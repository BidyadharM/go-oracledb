...
import (
	...
	"github.com/oracle/go-oracledb/driver/ttc"
)
...
func TestDriver_VARRAY_TypeShapeDebug(t *testing.T) {
	...
	var unusedAttributes, unusedSubtypes refcursor.RefCursor
...
func TestDriver_VARRAY_InsertSelectPLSQL(t *testing.T) {
	...
	var typ *adt.ObjectType
	err = conn.Raw(func(driverConn any) error {
		var ok bool
		physical, ok := driverConn.(*ttc.Connection)
		if !ok {
			return fmt.Errorf("unexpected driver connection %T", driverConn)
		}
		typ, err = physical.GetObjectType(ctx, typeName)
		return err
	})
	if err != nil {
	...
	var selected adt.ObjectCollection
...
func mustNumberVARRAY(t *testing.T, typ *adt.ObjectType, values ...int64) adt.ObjectCollection {
	t.Helper()
...
func assertNumberVARRAY(t *testing.T, collection adt.ObjectCollection, want ...int64) {
	t.Helper()
...
func TestDriver_VARRAY_PublicAPIs(t *testing.T) {
	...
	tests := []struct {
		...
		element string
		values  []any
		check   func(*testing.T, adt.ObjectCollection)
	}{
		{name: "varchar2", element: "VARCHAR2(30)", values: []any{"alpha", "beta"}, check: func(t *testing.T, c adt.ObjectCollection) {
			assertVARRAYValues(t, c, "alpha", "beta")
		}},
		{name: "raw", element: "RAW(16)", values: []any{[]byte{1, 2, 3}, []byte{0xFE, 0xED}, nil}, check: func(t *testing.T, c adt.ObjectCollection) {
...
func assertVARRAYValues(t *testing.T, collection adt.ObjectCollection, want ...any) {
	t.Helper()
	gotLen, err := collection.Len()
	if err != nil || gotLen != len(want) {
		t.Fatalf("VARRAY length = %d, %v; want %d", gotLen, err, len(want))
...
func TestDriver_VARRAY_GetObjectTypeOtherScalarTypes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			typ, err := adt.GetObjectType(ctx, conn, typeName)
			if err != nil {
				t.Fatalf("GetObjectType(%s): %v", typeName, err)
			}
			collection, err := typ.NewCollection()
